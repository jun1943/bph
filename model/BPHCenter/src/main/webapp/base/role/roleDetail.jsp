...
<html>
  ...
  <body>
    ...
    function AssignCheck() {
		 $.ajax({
			 url:"<%=basePath%>role/getModuleFunction.do",
			 type:"post",
			 dataType:"json",
			 data:{
				roleId:$("#roleId").val()				 
				 ...
				 if(msg.code==200){
					 if(msg.data != null){
						 ...
						 $(msg.data).each(function(){
							treeObj.checkNode(treeObj.getNodeByParam("id",this.modFunId, null), true, true);
						 });
					 }
				 }
			 }
      ...
      <div class="mid-form auto mt10"><!----表单---->
        ...
        <input id="roleId" type="hidden" value="${role.id}"/>
            ...
            <td>
              <table class="text-table">
                <tr>
                  ...
                  <td><input id="roleName" value="${role.name}" type="text" class="form-control w258 text" placeholder="等待输入..."></td>
          ...
          <tr>
            <td>角色备注：</td>
            <td>
               <textarea id="roleNote" class=" textarea w640 h260 mt12" >${role.note}</textarea>
            </td>
          </tr>
        </table>
        <hr class="hr mt20">
        <div>
          <a href="#" onclick="editRole()" class="button-red auto mt40">确认修改</a>
         <!--  <a href="#" onclick="AssignCheck()" class="button-red auto mt40">确认添加</a> -->
        </div>
      </div><!----表单结束---->
      <hr class="hr mt10">
</div> 
