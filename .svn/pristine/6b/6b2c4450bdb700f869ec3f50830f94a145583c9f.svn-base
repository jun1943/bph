...
//IP访问
//XTREE2_HTML_CONTEXT_ROOT = "/megaeye/";
//域名解析
XTREE2_HTML_CONTEXT_ROOT = "/";
//
...
// /////////////////////////////////////////////////////////////////////////////
var webFXTreeConfig = {
	rootIcon        : XTREE2_HTML_CONTEXT_ROOT+"webClient/js/control/xloadtree/images/folder.png",
	openRootIcon    : XTREE2_HTML_CONTEXT_ROOT+"webClient/js/control/xloadtree/images/openfolder.png",
	folderIcon      : XTREE2_HTML_CONTEXT_ROOT+"webClient/js/control/xloadtree/images/folder.png",
	openFolderIcon  : XTREE2_HTML_CONTEXT_ROOT+"webClient/js/control/xloadtree/images/openfolder.png",
	fileIcon        : XTREE2_HTML_CONTEXT_ROOT+"webClient/js/control/xloadtree/images/file.png",
	iIcon           : XTREE2_HTML_CONTEXT_ROOT+"webClient/js/control/xloadtree/images/I.png",
	lIcon           : XTREE2_HTML_CONTEXT_ROOT+"webClient/js/control/xloadtree/images/L.png",
	lMinusIcon      : XTREE2_HTML_CONTEXT_ROOT+"webClient/js/control/xloadtree/images/Lminus.png",
	lPlusIcon       : XTREE2_HTML_CONTEXT_ROOT+"webClient/js/control/xloadtree/images/Lplus.png",
	tIcon           : XTREE2_HTML_CONTEXT_ROOT+"webClient/js/control/xloadtree/images/T.png",
	tMinusIcon      : XTREE2_HTML_CONTEXT_ROOT+"webClient/js/control/xloadtree/images/Tminus.png",
	tPlusIcon       : XTREE2_HTML_CONTEXT_ROOT+"webClient/js/control/xloadtree/images/Tplus.png",
	plusIcon        : XTREE2_HTML_CONTEXT_ROOT+"webClient/js/control/xloadtree/images/plus.png",
	minusIcon       : XTREE2_HTML_CONTEXT_ROOT+"webClient/js/control/xloadtree/images/minus.png",
	blankIcon       : XTREE2_HTML_CONTEXT_ROOT+"webClient/js/control/xloadtree/images/blank.png",
...
_p.remove = function(oChild) {
	...
	if (si == oChild || oChild.contains(si)) {
		if (si.getFocused()) {
			this.select();
			window.setTimeout("WebFXTreeAbstractNode._onTimeoutFocus(\""
							+ this.id + "\")", 10);
		} else {
			this.select();
		}
...
_p.toHtml = function() {
	...
	var t = this.getTree();
	var hideLines = !t.getShowLines() || t == this.parentNode
			&& !t.getShowRootLines();
	...
	var childrenHtml = "<div class=\"webfx-tree-children"
			...
			+ "\">" + sb.join("") + "</div>";
	// alert("<div class=\"webfx-tree-item\" id=\"" +this.id + "\"" +
	// this.getEventHandlersHtml() + ">" +this.getRowHtml() +childrenHtml
	// +"</div>");
	return "<div class=\"webfx-tree-item\" id=\"" + this.id + "\""
			+ this.getEventHandlersHtml() + ">" + this.getRowHtml()
			+ childrenHtml + "</div>";
};

function chkClick(chkObj) {
	var radName = chkObj.id + "-radio";
...
function radClick(radObj) {
	var radName = radObj.name;
	var arr = radName.split("-");
	var chkId = arr[0];
...
_p.getRowHtml = function() {
	...
	if (this.chk != null) {
		chkOrRadioHtml = "<input type=\"checkbox\" id=\"" + this.chk
				+ "\" name=\"chk\"  value=\"" + this.chk
				+ "\" onclick=\"chkClick(this);\">";
	} else if (this.radio != null) {
		var arr = this.radio.split(";");
		chkOrRadioHtml = "<input type=\"radio\" id=\"" + arr[2] + "\" name=\""
				+ arr[0] + "\" value = \"" + arr[1]
				+ "\" onclick=\"radClick(this);\">";
	}

	if (this.rootHidden == "true") {
		return "<div class=\"" + this.getRowClassName()
				+ "\" style=\"padding-left:"
				+ Math.max(0, (this.getDepth() - 1) * this.indentWidth)
				+ "px; display: none;\">" + this.getExpandIconHtml() +
				// "<span class=\"webfx-tree-icon-and-label\">" +
				this.getIconHtml() + chkOrRadioHtml + this.getLabelHtml() +
				// "</span>" +
				"</div>";
	} else {
		return "<div class=\"" + this.getRowClassName()
...
_p.getLabelHtml = function() {
	...
	return "<a href=\""
			+ webFXTreeHandler.textToHtml(this._getHref())
...
_p.getExpandIconSrc = function() {
	...
	if (this.hasChildren()) {
		var bits = 0;
		/*
		 * Bitmap used to determine which icon to use 1 Plus 2 Minus 4 T Line 8
		 * L Line
		 */
...
_p._onmousedown = function(e) {
	...
	this.select();
	if (/* !/webfx-tree-item-label/.test(el.className) && */!webFXTreeHandler.opera) { // opera
		// cancels
		// the
		// click
		// if
...
_p.getExpanded = function() {
	return !this.showRootNode
			|| WebFXTreeAbstractNode.prototype.getExpanded.call(this);
};
...
_p.getIconSrc = function() {
	var behavior = this.getTree()
			? this.getTree().getBehavior()
			: webFXTreeConfig.defaultBehavior;
...
_p.getEventHandlersHtml = function() {
	return " onmousedown=\"return webFXTreeHandler.handleEvent(event)\" "
	...
	/*
	 * " onclick=\"return webFXTreeHandler.handleEvent(event)\" " +
...
function WebFXTreeItem(sText, oAction, eParent, sIcon, sOpenIcon, chk, radio) {
	WebFXTreeAbstractNode.call(this, sText, oAction);
	if (sIcon)
		this.icon = sIcon;
	if (sOpenIcon)
