...
WebFXLoadTree.createLoadingItem = function () {
	return new WebFXTreeItem("Loading...", null, null,
							 "");
};
...
function WebFXLoadTreeItem(sText, sXmlSrc, oAction, eParent, sIcon, sOpenIcon, chk, radio) {
	//alert ("chk2:"+chk);
	WebFXTreeItem.call(this, sText, oAction, eParent, sIcon, sOpenIcon, chk, radio);
...
WebFXLoadTree.createItemFromElement = function (oNode) {
	...
	//alert ("chk1:"+jsAttrs.chk);
	var jsNode = new WebFXLoadTreeItem(jsAttrs.html || "", jsAttrs.src, action,
									   null, jsAttrs.icon, jsAttrs.openIcon, jsAttrs.chk, jsAttrs.radio);
...
WebFXLoadTree.documentLoaded = function (jsNode) {
	...
	if(!doc || doc.parserError && doc.parseError.errorCode != 0 || !doc.documentElement) {
		...
		for (var i = 0; i < l; i++) {
			if (cs[i].tagName == "tree") {
				...
				//console.dir(cs[i]);
				if (cs[i].getAttribute("text") == "false") {
					//alert('----------false');
	...
	if (jsNode.errorText != "") {
		jsNode._loadingItem.icon = "/webClient/pages/images/exclamation.16.gif";
